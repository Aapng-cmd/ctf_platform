...
def insert_hashed_string(hashed_string):
    try:
        # Establish a database connection
        connection = pymysql.connect(
            host='localhost',  # Change to your host
            user='user_site',  # Change to your username
            password='password',  # Change to your password
            database='site',  # Change to your database name
            charset='utf8mb4'
        )

        with connection.cursor() as cursor:
            # Create the insert query
            insert_query = "INSERT INTO invitations (invite_code) VALUES (%s)"
            cursor.execute(insert_query, (hashed_string,))
            connection.commit()  # Commit the transaction
            print("Hashed string inserted successfully.")
...
hashed_string = hash_string(generate_random_string(100))
insert_hashed_string(hashed_string)
